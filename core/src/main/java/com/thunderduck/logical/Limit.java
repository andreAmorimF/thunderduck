...
public class Limit extends LogicalPlan {
    ...
    public String toSQL(SQLGenerator generator) {
        ...
        if (offset > 0) {
            return String.format("SELECT * FROM (%s) AS %s LIMIT %d OFFSET %d",
                childSQL, generator.generateSubqueryAlias(), limit, offset);
        } else {
            return String.format("SELECT * FROM (%s) AS %s LIMIT %d",
                childSQL, generator.generateSubqueryAlias(), limit);
        }
    }

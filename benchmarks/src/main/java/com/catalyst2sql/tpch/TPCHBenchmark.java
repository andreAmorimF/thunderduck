...
import com.catalyst2sql.optimizer.QueryOptimizer;
...
public class TPCHBenchmark {
    ...
    /**
     * Enable/disable query optimization for comparison.
     */
    @Param({"true", "false"})
    ...
    public void setup() {
        ...
        // Initialize optimizer (with or without rules)
        if (enableOptimization) {
            this.optimizer = new QueryOptimizer();
        } else {
            this.optimizer = new QueryOptimizer(Collections.emptyList(), 1);
        }
    ...
    @Benchmark
    public LogicalPlan benchmarkQ1_Optimization() {
        return optimizer.optimize(q1Plan);
    }

    @Benchmark
    public String benchmarkQ1_EndToEnd() {
        LogicalPlan optimized = optimizer.optimize(q1Plan);
    ...
    @Benchmark
    public LogicalPlan benchmarkQ2_Optimization() {
        return optimizer.optimize(q2Plan);
    }
    ...
    @Benchmark
    public LogicalPlan benchmarkQ3_Optimization() {
        return optimizer.optimize(q3Plan);
    }
    ...
    @Benchmark
    public LogicalPlan benchmarkQ6_Optimization() {
        return optimizer.optimize(q6Plan);
    }
    ...
    public static void main(String[] args) {
        ...
        benchmark.enableOptimization = true;
        benchmark.setup();
        ...
        System.out.println("\n=== TPC-H Q1 Optimized ===");
        try {
            LogicalPlan optimized = benchmark.benchmarkQ1_Optimization();
            String optimizedSql = benchmark.generator.generate(optimized);
            System.out.println("Optimized SQL:");
            System.out.println(optimizedSql);
        } catch (UnsupportedOperationException e) {
            System.out.println("Optimization not yet fully implemented: " + e.getMessage());
            System.out.println("Benchmark framework is ready - optimization features will be added in future tasks.");
        }
